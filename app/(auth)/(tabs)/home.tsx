...
const DashboardScreen = () => {
  ...
  const { isLoading, countries, error: errorFetchingCountries } = useFetchCountries();
  const {
        ...
        options={{
          headerLeft: () => (
            <View style={{ gap: 4, marginLeft: 16 }}>
              <Text style={{ fontSize: 16, fontFamily: 'inter' }}>Welcome back</Text>
          ...
          headerRight: () => (
            ...
            >
              <View style={{ borderRadius: 4, overflow: 'hidden' }}>
                <CountryFlag isoCode={selectedCountry.countryCurrency!.slice(0, 2)} size={16} />
              </View>
      ...
      <View style={[styles.content, { paddingTop: top + 5 }]}>
        <Animated.View
          onLayout={(event) => {
            ...
            if (width.value !== layoutWidth) {
              ...
              animatedTabPosition.value = (layoutWidth / period.length) * activeTabIndex;
            }
          }}
          ...
          {period.map((item, index) => (
            <AnimatedTouchableOpacity
              ...
              onPress={() => {
                ...
                animatedTabPosition.value = withTiming((width.value / period.length) * index, {
                  duration: 300,
                });
              }}
              hitSlop={25}
        ...
        <View style={defaultStyles.card}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            <View style={{ flexDirection: 'row', gap: 8 }}>
              <View>
                <Text style={{ fontSize: 18, fontFamily: 'inter-sb' }}>Revenue</Text>
                <Text style={{ fontSize: 24, color: Colors.dark, fontFamily: 'inter-b' }}>
                  {selectedCountry.countrySymbol}
                  {dashboard?.revenue != null ? dashboard.revenue.toLocaleString() : '0.00'}
                </Text>
              </View>
          ...
          {dashboard?.bestSellers?.length > 0 ? (
            ...
            <EmptyState message="No order activity for this period" iconName="cart-outline" />
          )}
        </View>
        <View style={{ flexDirection: 'row', gap: 6 }}>
          <View style={[defaultStyles.card, { width: (windowWidth - 40) / 2, paddingVertical: 6 }]}>
            <View style={{ justifyContent: 'center', flexGrow: 1, gap: 12 }}>
              <Text style={{ fontSize: 20, fontFamily: 'inter' }}>Expected Revenue</Text>
              <Text style={{ fontSize: 20, fontFamily: 'inter-b' }}>
                {selectedCountry.countrySymbol}
          ...
          <View style={[defaultStyles.card, { width: (windowWidth - 40) / 2, paddingVertical: 6 }]}>
            <View style={{ justifyContent: 'center', flexGrow: 1, gap: 12 }}>
              <View style={{ gap: 4 }}>
                <Text style={{ fontSize: 18, fontFamily: 'inter', textAlign: 'center' }}>
                  Total{'\n'}Orders(Amount)
                </Text>
                <Text style={{ fontSize: 22, fontFamily: 'inter-b', textAlign: 'center' }}>
                  {selectedCountry.countrySymbol}
              ...
              <View>
                <Text style={{ fontSize: 14, fontFamily: 'inter', textAlign: 'center' }}>
                  Total # of Orders:{' '}
        ...
        <View style={defaultStyles.card}>
          <View style={{ flexDirection: 'row', gap: 6, marginBottom: 6, alignItems: 'center' }}>
            ...
            <Text style={{ fontSize: 24, fontFamily: 'inter-s' }}>Best Sellers</Text>
          </View>
          {dashboard?.bestSellers?.length > 0 ? (
        ...
        <View style={defaultStyles.card}>
          <View style={{ flexDirection: 'row', gap: 6, marginBottom: 6, alignItems: 'center' }}>
            <AntDesign name="exclamationcircleo" size={24} color="black" />
            <Text style={{ fontSize: 24, fontFamily: 'inter-s' }}>Order Status</Text>
          </View>
